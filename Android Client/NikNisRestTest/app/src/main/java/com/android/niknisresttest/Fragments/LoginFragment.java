...
public class LoginFragment extends Fragment {
    ...
    private void loginUser() {
        ...
        if (TextUtils.isEmpty(Email)){
            ...
            Call<Void> userCall = MainActivity.serviceApi.doLogin(user);
            userCall.enqueue(new Callback<Void>() {
                @Override
                public void onResponse(Call<Void> call, Response<Void> response) {
                    if (response.headers().get("Authorization") != null){
                        MainActivity.appPreference.setLoginStatus(true); // set login status in sharedPreference
                        MainActivity.appPreference.setToken(response.headers().get("Authorization"));
                        System.out.println("--------"+MainActivity.appPreference.getToken());
                        loginFromActivityListener.login(MainActivity.appPreference.getToken());
                ...
                public void onFailure(Call<Void> call, Throwable t) {
                    System.out.println("=nnnn======="+t.fillInStackTrace());
                }
